  ...
  return (
    <tr className="animate-pulse">
      {Array.from({ length: 6 }).map((_, i) => (
        <td key={i} className="px-4 py-3"><div className="h-4 bg-slate-200 rounded w-full" /></td>
      ))}
    </tr>
  );
}
...
export default function PanelAdmin({ onVerSitio }: { onVerSitio?: () => void }) {
  ...
  // Recargar reservas del día cada vez que se vuelve al tab "hoy"
  useEffect(() => {
    if (tab === 'hoy') fetchReservas();
  }, [tab, fetchReservas]);
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      <header className="bg-white border-b border-slate-200 px-6 py-4 flex items-center justify-between">
        <div>
          <h1 className="text-xl font-semibold text-slate-800">{user?.nombre} {user?.apellido}</h1>
          <p className="text-sm text-slate-500 capitalize">{user?.rol?.toLowerCase()}</p>
        </div>
        <div className="flex items-center gap-3">
          {onVerSitio && (
            <button onClick={onVerSitio} className="text-sm px-3 py-1.5 rounded-lg border border-slate-200 text-slate-600 hover:bg-slate-50">
              🌐 Ver sitio
            </button>
          )}
      ...
      <main className="px-6 py-8 max-w-7xl mx-auto">
          ...
          {(
            ...
            <button key={k} onClick={() => setTab(k)}
              className={`px-3 py-2 rounded-xl text-sm border transition-colors ${
                tab === k ? 'bg-slate-900 text-white border-slate-900' : 'bg-white text-slate-700 border-slate-200 hover:bg-slate-50'
              }`}>
              {label}
            </button>
          ))}
        ...
        {tab === 'hoy' && (
          <>
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-lg font-semibold text-slate-700">Reservas del día</h2>
              <button onClick={fetchReservas} className="text-sm px-3 py-1.5 rounded-lg border border-slate-200 hover:bg-slate-50">
                🔄 Actualizar
              </button>
            </div>
            {error ? (
                ...
                <table className="min-w-full text-sm text-slate-700">
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {loading ? (
                      ...
                      <tr><td colSpan={6} className="px-4 py-8 text-center text-slate-400">No hay reservas para hoy.</td></tr>
                    ) : reservas.map(r => (
                      <tr key={r.id_reserva} className="hover:bg-slate-50 transition-colors">
                        ...
                        <td className="px-4 py-3">
                          <span className={`inline-block px-2 py-0.5 rounded-full text-xs font-medium ${ESTADO_BADGE[r.estado] ?? 'bg-slate-100 text-slate-600'}`}>
                            {r.estado}
                          </span>
                        </td>
        ...
        {tab === 'clientes'   && <ClientesCRUD />}
        {tab === 'usuarios'   && <UsuariosCRUD />}
        {tab === 'reservas'   && <ReservasCRUD />}
        {tab === 'bitacora'   && <BitacoraDashboard />}
        {tab === 'sps'        && <StoredProcedures />}
        {tab === 'reportes'   && <ModuloReportes />}
      </main>
    </div>
  );
}
